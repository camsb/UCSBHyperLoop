...
void flipE()
{
  Chip_GPIO_SetPinOutHigh( LPC_GPIO, DATAPORTS[ EPIN ], DATAPINS[ EPIN ] );
  Chip_GPIO_SetPinOutLow( LPC_GPIO, DATAPORTS[ EPIN ], DATAPINS[ EPIN ] );
...
void setData( int v )
{
  ...
  for( i = 0; i < 8; i++ )
  {
    ...
    if( highLow == 0 )
      Chip_GPIO_SetPinOutLow(  LPC_GPIO, DATAPORTS[ 7 - i ], DATAPINS[ 7 - i ] );
    else
      Chip_GPIO_SetPinOutHigh( LPC_GPIO, DATAPORTS[ 7 - i ], DATAPINS[ 7 - i ] );
  }
...
void setDataPinsToOutputAndLow()
{
  ...
  {
    Chip_GPIO_SetPinDIROutput( LPC_GPIO, DATAPORTS[ i ], DATAPINS[ i ] );
    Chip_GPIO_SetPinOutLow(    LPC_GPIO, DATAPORTS[ i ], DATAPINS[ i ] );
  }
...
{
  if( v == LOW )
    Chip_GPIO_SetPinOutLow( LPC_GPIO, DATAPORTS[ RSPIN ], DATAPINS[ RSPIN ] );
  else
    Chip_GPIO_SetPinOutHigh( LPC_GPIO, DATAPORTS[ RSPIN ], DATAPINS[ RSPIN ] );
...
{
  if( v == LOW )
    Chip_GPIO_SetPinOutLow( LPC_GPIO, DATAPORTS[ RWPIN ], DATAPINS[ RWPIN] );
  else
    Chip_GPIO_SetPinOutHigh( LPC_GPIO, DATAPORTS[ RWPIN ], DATAPINS[ RWPIN ] );
